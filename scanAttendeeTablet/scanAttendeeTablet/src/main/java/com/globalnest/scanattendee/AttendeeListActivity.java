...
@SuppressLint("NewApi")
public class AttendeeListActivity extends BaseActivity implements OnClickListener, DownloadResultReceiver.Receiver {
	// tab bar
		...
		attendee_view.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> _adapter, View view, int position, long index) {
				try {
					...
					Intent att_activity = new Intent(AttendeeListActivity.this, AttendeePagerSampleActivity.class);
	...
	public void OntabClick(boolean isBuyer) {
		try {
			...
			if (isBuyer) {
				...
				//UpdateAttendeeCursor();
				//SortFunction(R.id.editsearchrecord, is_buyer);
				if (!NullChecker(item_pool_id).isEmpty()&&NullChecker(activity_name).isEmpty()) {
					UpdateAttendeeCursor();
				} else {
					SortFunction(1, is_buyer);
				}
	...
	private String getOfflineJsonBody() {
		...
		for (OfflineScansObject scanObj : offlineList) {
			...
			try {
				...
				obj.put("freeSemPoolId", scanObj.Checkinout);
