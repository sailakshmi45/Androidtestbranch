...
public class EditCard extends EditText {
    ...
    private void addMagic() {
        ...
        addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int position, int before, int action) {
                AppUtils.displayLog("--------------Position and action value---------",":"+position+" : "+action);
                if (action == 1) {
                    if (type.equals("UNKNOWN") || type.equals("Visa") || type.equals("Discover") || type.equals("JCB")) {
                        if (position == 3 || position == 8 || position == 13) {
                            AppUtils.displayLog("--------------is have Dash---------",":"+(!s.toString().endsWith("-")));
                            if (!s.toString().endsWith("-")) {
                                //append("-");
                                s = s+"-";
                                setText(s);
                                if(s.length()<20) {//kept for indexbond execption
                                    setSelection(s.length());
                                }
                                AppUtils.displayLog("-------------Get Text---------",":"+getText().toString());
                            }
    ...
    private void changeIcon() {
        ...
        if (s.startsWith("4") || s.matches(CardPattern.VISA)) {
            ...
            setCompoundDrawablesWithIntrinsicBounds(0, 0, R.drawable.card_new, 0);
            type = "UNKNOWN";
        }
    }
