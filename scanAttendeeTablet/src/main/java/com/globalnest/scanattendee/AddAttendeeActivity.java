...
public class AddAttendeeActivity extends BaseActivity{
	...
	private String getSellTicketRequest(){
		...
		try {
			...
			buyer_obj.put("CompanyName", buyerinfo.getCompany());
			...
			buyer_obj.put("Email", buyerinfo.getEmail().toLowerCase());
			...
			line_item_object.put("CompanyName",	buyerinfo.getCompany());
			line_item_object.put("Email",	buyerinfo.getEmail().toLowerCase());
			...
			line_item_object.put("PhoneNumber", buyerinfo.getMobile());
			if(Util.getselfcheckinbools(Util.ISSELFCHECKIN)){
