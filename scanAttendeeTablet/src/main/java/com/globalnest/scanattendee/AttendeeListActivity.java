...
@SuppressLint("NewApi")
public class AttendeeListActivity extends BaseActivity implements OnClickListener, DownloadResultReceiver.Receiver,SocketListener {
	// tab bar
	...
	private ListView buyers_list, attendee_view;
	...
	private String requestType = "", filter_by = "ALL",badgefilter_by="ALL",rsvpfilter_by="ALL", delete_attendeeId = "",attendeeRecords="";
	boolean isFilter, isCheckedIn = false, isDialogVisible = false, isError, is_buyer = false, isOffline = false,iscancelpressed=false;
	...
	private ArrayAdapter<String> badgearray_adapter,rsvpstatus_adapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/*txt_save.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				Intent i=new Intent(AttendeeListActivity.this,PrintAttendeesListActivity.class);
				i.putExtra("","");
				startActivity(i);
			}
		});*/
		...
		//connectIOSocket();
		if(externalSettings.allow_livedata) {
			//if(!mSocket.isActive())
			connectIOSocket();// added new for java socket
		}else{
			dissconnectIOSocket();
		}
		if(externalSettings.group_checkin) {
		...
		txt_save.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(tickets_register.size()>0) {
					...
					AlertDialogCustom d = new AlertDialogCustom(AttendeeListActivity.this);
		...
		if(session_id != null && session_id.length() > 0){
			...
			dialog = new ProgressDialog(AttendeeListActivity.this);
			dialog.setMessage("Please wait loading attendees...");
			dialog.setCancelable(false);
			if (Util.isMyServiceRunning(DownloadService.class, AttendeeListActivity.this)) {
		...
		img_close.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				hideSoftKeyboard(AttendeeListActivity.this);
		...
		img_camera.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (Util.db.getGroupCount(checked_in_eventId) == 0) {
				...
				} else if (Util.isMyServiceRunning(DownloadService.class, AttendeeListActivity.this)) {
					...
					if(AppUtils.isCamPermissionGranted(AttendeeListActivity.this)){
						Intent i = new Intent(AttendeeListActivity.this, BarCodeScanActivity.class);
						i.putExtra(Util.SCAN, "scan_layout");
						i.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP|Intent.FLAG_ACTIVITY_CLEAR_TASK);
						//startActivityForResult(i, Util.DASHBORD_ONACTIVITY_REQ_CODE);
						startActivity(i);
					}else {
						AppUtils.giveCampermission(AttendeeListActivity.this);
					}
				}
		...
		search_view.setOnEditorActionListener(new TextView.OnEditorActionListener() {
			...
			public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
				if (actionId == EditorInfo.IME_ACTION_SEARCH&&!search_view.getText().toString().trim().isEmpty()) {
					...
					Toast.makeText(AttendeeListActivity.this, "Please Enter Text!", Toast.LENGTH_LONG).show();
				}
				return false;
			}
		});
		...
		attendee_view.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> _adapter, View view, int position, long index) {
				try {
					...
					// Log.i("AttendeeListActivity", "Order_Id " +
					// c1.getString(c1.getColumnIndex("Order_Id")));
					Intent att_activity = new Intent(AttendeeListActivity.this, AttendeeExpandableActivity.class);

					//Intent att_activity = new Intent(AttendeeListActivity.this, AttendeeDetailActivity.class);
					...
					att_activity.putExtra("Tktprofile_Id", c1.getString(c1.getColumnIndex("Tktprofile_Id")));
					att_activity.putExtra(DBFeilds.ATTENDEE_FIRST_NAME, c1.getString(c1.getColumnIndex(DBFeilds.ATTENDEE_FIRST_NAME)));
					att_activity.putExtra(DBFeilds.ATTENDEE_LAST_NAME, c1.getString(c1.getColumnIndex(DBFeilds.ATTENDEE_LAST_NAME)));
					att_activity.putExtra(DBFeilds.ATTENDEE_EMAIL_ID, c1.getString(c1.getColumnIndex(DBFeilds.ATTENDEE_EMAIL_ID)));
					att_activity.putExtra(DBFeilds.ATTENDEE_ITEM_ID, c1.getString(c1.getColumnIndex(DBFeilds.ATTENDEE_ITEM_ID)));
		...
		img_addticket.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				//connectWebSocket();
				//connectIOSocket();
				//Util.db.createvirtualtable();
				Intent addattendee = new Intent(AttendeeListActivity.this, AddAttendeeActivity.class);
		...
		img_refund_history.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				doRequest();*/
				Intent i = new Intent(AttendeeListActivity.this, OfflineDataSyncActivty.class);
	...
	public void OntabClick(boolean isBuyer) {
		try {
			...
			if (isBuyer) {
				...
				else if (!NullChecker(item_pool_id).isEmpty()&&NullChecker(activity_name).isEmpty()) {
					//updateCheckinView();
					UpdateAttendeeCursor();
				} else {
					SortFunction(1, is_buyer);
	...
	protected void onResume() {
		try {
			...
			if(!externalSettings.allow_livedata) {
				//if(!mSocket.isActive())
				dissconnectIOSocket();
			}else if(Boolean.valueOf(BaseActivity.checkedin_event_record.Events.Real_Time_Checkin__c)&&externalSettings.allow_livedata){
        ...
        if (!NullChecker(activity_name).isEmpty()) {
			...
			if (NullChecker(Util.db.getSwitchedONGroup(checked_in_eventId).Id).isEmpty() && NullChecker(session_id).isEmpty()) {
				...
				if (adapter != null) {
					...
					adapter = new AttendeeAdapter(AttendeeListActivity.this, att_cursor);
					//attendee_view.setVisibility(View.VISIBLE);
					attendee_view.setAdapter(adapter);
					attendee_view.setVisibility(View.VISIBLE);
					//adapter.changeCursor(att_cursor);
			...
			else if ((!NullChecker(session_id).isEmpty()) && (!isOffline) && (!is_buyer) && NullChecker(item_pool_id).isEmpty()) {//!NullChecker(Util.db.getSwitchedONGroup(checked_in_eventId).Id).isEmpty() ||
				...
				att_cursor = Util.db.getAttendeeDataCursor(WHERE_EVENTID
					...
					if (att_cursor == null || whereClause_name.isEmpty()) {
						...
						att_cursor = Util.db.getAttendeeDataCursor(whereClause_name//WHERE_EVENTID
								, itempool_id, activity_name, group_id);
					} else if (att_cursor.getCount() == 0 && att_cursor != null) {
						att_cursor.close();
						att_cursor = Util.db.getAttendeeDataCursor(whereClause_name//WHERE_EVENTID
								, itempool_id, activity_name, group_id);
					}
	...
	private void UpdateBuyerCursor() {
		...
		if (buyer_cursor.getCount() > 0) {
			txt_noattendee.setVisibility(View.GONE);
			buyer_adapter = new BuyerCursorAdapter(AttendeeListActivity.this, buyer_cursor, AttendeeListActivity.this,
	...
	private void UpdateAttendeeCursor() {
		try{
			...
			if (search_view.getText().toString().trim().isEmpty()) {
				...
				if(!NullChecker(filter_by).isEmpty()&&!NullChecker(filter_by).equalsIgnoreCase("ALL")){
					if (list_offline != null && offlineList.size() > 0) {
						if (!NullChecker(activity_name).isEmpty()) {
							tab_bar.setVisibility(View.VISIBLE);
							buyer_tab.setVisibility(View.GONE);
	...
	public void doRequest() {
		if(isOnline()) {
			...
			if (requestType.equalsIgnoreCase("CheckInCount") || requestType.equalsIgnoreCase("CheckInCountRefresh")) {
				...
				postMethod = new HttpPostData(msg, url, null, access_token, AttendeeListActivity.this);
				postMethod.execute();
				/*DownloadResultReceiver mReceiver = new DownloadResultReceiver(new Handler());
				mReceiver.setReceiver(AttendeeListActivity.this);
				Intent intent = new Intent(Intent.ACTION_SYNC, null, AttendeeListActivity.this, DownloadService.class);
				...
				intent.putExtra(DownloadService.ACTIVITY_NAME, AttendeeListActivity.class.getName());
			...
			} else if (requestType.equals("groupcheckin")) {
				postMethod = new HttpPostData("",setTicketCheckinUrl(), makeCheckin().toString(), access_token, AttendeeListActivity.this);
				postMethod.execute();
				...
				dialog.setOnShowListener( new DialogInterface.OnShowListener() {
					...
					public void onShow(DialogInterface arg0) {
						dialog.getButton(AlertDialog.BUTTON_NEGATIVE).setTextColor(getResources().getColor(R.color.blue_text_color));
						...
						dialog.getButton(AlertDialog.BUTTON_NEGATIVE).setBackgroundDrawable(getResources().getDrawable(R.drawable.card_layout));
					}
				});
				dialog.show();
				DownloadResultReceiver mReceiver = new DownloadResultReceiver(new Handler());
				...
				postMethod = new HttpPostData("Loading Search...", url, null, access_token, AttendeeListActivity.this);
				postMethod.execute();
				...
				postMethod = new HttpPostData("Please Wait...",
						...
						getOfflineJsonBody().toString(), accessToken, AttendeeListActivity.this);
				 ...
				 * sfdcddetails.access_token, AttendeeListActivity.this);
				 */
				String _url = sfdcddetails.instance_url + WebServiceUrls.SA_GET_TICKET_LIST + "Event_id="
						+ checked_in_eventId;
				postMethod = new HttpPostData("Loading Tickets...", _url, null, access_token, AttendeeListActivity.this);
				postMethod.execute();
			}
		}else{
	...
	public JSONArray makeCheckin() {
		try {
			...
			String checked_time = getESTFormat();
			for (String key : tickets_register.keySet()) {
	...
	private void doCheckBackgroundService() {
		CheckServiceRunning checkReq = new CheckServiceRunning(AttendeeListActivity.this);
		checkReq.execute("");
	}
		...
		try {
			if (!isValidResponse(response)) {
				...
				if(requestType.equalsIgnoreCase("CheckInCount") || requestType.equalsIgnoreCase("CheckInCountRefresh")){
					...
					if(objsessionGson != null){
						sessionCheckIns = objsessionGson.totalItemschkQty;
			...
			UpdateAttendeeCursor();*/
					...
					if (totalorderlisthandler.TotalLists.size() > 0) {
						...
						Toast.makeText(AttendeeListActivity.this, "No Attendee Found!", Toast.LENGTH_LONG).show();
					}
	...
	private void updateCheckinViewforSocket() {
		if(externalSettings.allow_livedata) {
			runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					setEnableSessionHeadder();
					Parcelable state = attendee_view.onSaveInstanceState();
					if (search_view.getText().toString().trim().isEmpty()) {
						String groupid = NullChecker(Util.db.getSwitchedONGroup(checked_in_eventId).Id);
						String itempool_id = "";
						if (!groupid.isEmpty()) {
						...
						if (!whereClause_name.trim().isEmpty() && att_cursor != null) {
							att_cursor = Util.db.getAttendeeDataCursor(whereClause_name, item_pool_id, activity_name, session_id);
							adapter.changeCursor(att_cursor);
							attendee_view.onRestoreInstanceState(state);
						} else {
							if (att_cursor == null) {
								 ...
								 * "'"));//ORDER BY CheckedInDate DESC
								 */
							} else if (att_cursor != null && att_cursor.getCount() > 0) {
								adapter = new AttendeeAdapter(AttendeeListActivity.this, att_cursor);
								attendee_view.setAdapter(adapter);
								att_cursor = Util.db.getAttendeeDataCursor(WHERE_EVENTID + attendeeRecords + ORDERBY_CASE, itempool_id, activity_name, groupid);
								adapter.changeCursor(att_cursor);
	...
	private void loadAttendeesInBackground() {
		...
		/*if (isOnline()) {
			if (totalcount < Util.dashboardHandler.totalOrders) {
				order_cursor.close();
				// Log.i("AttendeeListActivity", "---Downloading orders---");
				startService(intent);
			} else {
				order_cursor.close();
				// Log.i("AttendeeListActivity", "---All Records are
				// downloaded---");
			}
		}*/
	...
	public void searchBuyer(boolean isFromService) {
		...
		if (!search_view.getText().toString().trim().isEmpty()) {
			...
			if (buyer_cursor.getCount() > 0) {
				buyer_adapter = new BuyerCursorAdapter(AttendeeListActivity.this, buyer_cursor,
						AttendeeListActivity.this, sfdcddetails, false, checked_in_eventId, NullChecker(session_id));
				buyers_list.setAdapter(buyer_adapter);
			} else {
				txt_noattendee.setVisibility(View.VISIBLE);
	...
	private void SortFunction(int id, boolean isBuyer) {
		try {
			String wherebadgestatus = "", wherersvpstatus = "", group_id = "", itempool_id = "";
			if (att_cursor != null) {
				att_cursor.close();
			}
			if (id == 1) {
				...
				if(!rsvpfilter_by.equalsIgnoreCase("ALL")){
					wherersvpstatus = " AND "+DBFeilds.ATTENDEE_RSVP_STATUS+"= '"+rsvpfilter_by+"'";
				}
				if (!isFilter) {
					whereClause_name = WHERE_EVENTID+ wherebadgestatus +wherersvpstatus +attendeeRecords+ " ORDER BY "+DBFeilds.ATTENDEE_FIRST_NAME+" COLLATE NOCASE"; //"' ORDER BY "+DBFeilds.ATTENDEE_FIRST_NAME+" COLLATE NOCASE ";
					if (filter_by.equals("CHECKED IN")) {
						whereClause_name = WHERE_EVENTID +" AND Attendee_Id IN "
								+ innerQueryString("true") + wherebadgestatus +wherersvpstatus +attendeeRecords+ " ORDER BY "+DBFeilds.ATTENDEE_FIRST_NAME+" COLLATE NOCASE";
					} else if (filter_by.equals("ALL"))
						whereClause_name = WHERE_EVENTID + wherebadgestatus +wherersvpstatus+attendeeRecords+ " ORDER BY "+DBFeilds.ATTENDEE_FIRST_NAME+" COLLATE NOCASE";//+" ORDER BY "+DBFeilds.ATTENDEE_FIRST_NAME+" COLLATE NOCASE ";
					else if (filter_by.equals("CHECKED OUT")) {
						whereClause_name = WHERE_EVENTID +" AND Attendee_Id IN "
								+ innerQueryString("false") + wherebadgestatus +wherersvpstatus +attendeeRecords+ " ORDER BY "+DBFeilds.ATTENDEE_FIRST_NAME+" COLLATE NOCASE ";
					} else if (filter_by.equalsIgnoreCase("NOT REPORTED")) {
						whereClause_name = WHERE_EVENTID +" AND Attendee_Id NOT IN "
								+ innerQueryString("true") + wherebadgestatus +wherersvpstatus+attendeeRecords+ " ORDER BY "+DBFeilds.ATTENDEE_FIRST_NAME+" COLLATE NOCASE ";
					}/*else {
						whereClause_name = WHERE_EVENTID +" AND Attendee_Id IN "
								+ innerQueryString("") + wherebadgestatus +wherersvpstatus +attendeeRecords+ ORDERBY_CASE;
					}*/
					...
					if (filter_by.equals("CHECKED IN")) {
						whereClause_name = WHERE_EVENTID +" AND Attendee_Id IN "
								+ innerQueryString("true") + wherebadgestatus +wherersvpstatus +attendeeRecords+ ORDERBY_CASE;
					} else if (filter_by.equals("ALL")) {
						whereClause_name = WHERE_EVENTID + wherebadgestatus +wherersvpstatus +attendeeRecords+ ORDERBY_CASE;
					} else if (filter_by.equals("CHECKED OUT")) {
						whereClause_name = WHERE_EVENTID +" AND Attendee_Id IN "
								+ innerQueryString("false") + wherebadgestatus +wherersvpstatus +attendeeRecords+ ORDERBY_CASE;
					} else if (filter_by.equalsIgnoreCase("NOT REPORTED")) {
						whereClause_name = WHERE_EVENTID +" AND Attendee_Id NOT IN "
								+ innerQueryString("") + wherebadgestatus +wherersvpstatus +attendeeRecords+ ORDERBY_CASE;
					}
				...
				if (!isFilter) {
					...
					if (filter_by.equals("CHECKED IN")) {
						whereClause_name = WHERE_EVENTID +attendeeRecords+" AND Attendee_Id IN "
								+ innerQueryString("true") + wherebadgestatus +wherersvpstatus +attendeeRecords+ " ORDER BY "+DBFeilds.ATTENDEE_LAST_NAME+" COLLATE NOCASE";
					} else if (filter_by.equals("ALL"))
						whereClause_name = " Where Event_Id='" + checked_in_eventId
								+ "'" + wherebadgestatus +wherersvpstatus +attendeeRecords+ " ORDER BY "+DBFeilds.ATTENDEE_LAST_NAME+" COLLATE NOCASE ";
					else if (filter_by.equals("CHECKED OUT")) {
						whereClause_name = WHERE_EVENTID +" AND Attendee_Id IN "
								+ innerQueryString("false") + wherebadgestatus +wherersvpstatus +attendeeRecords+ " ORDER BY "+DBFeilds.ATTENDEE_LAST_NAME+" COLLATE NOCASE ";
					} else if (filter_by.equalsIgnoreCase("NOT REPORTED")) {
						whereClause_name = WHERE_EVENTID +" AND Attendee_Id NOT IN "
								+ innerQueryString("true") + wherebadgestatus +wherersvpstatus +attendeeRecords+ " ORDER BY "+DBFeilds.ATTENDEE_LAST_NAME+" COLLATE NOCASE ";
					}
					...
					if (filter_by.equals("CHECKED IN")) {
						whereClause_name = WHERE_EVENTID +" AND isCheckin= '" + "true" + "'" + wherebadgestatus +wherersvpstatus +attendeeRecords+ ORDERBY_CASE;
					} else if (filter_by.equals("ALL")) {
						whereClause_name = WHERE_EVENTID + wherebadgestatus +wherersvpstatus +attendeeRecords+ ORDERBY_CASE;
					} else if (filter_by.equals("NOT CHECKED IN")) {
						whereClause_name = WHERE_EVENTID +" AND isCheckin= '" + "false" + "'" + wherebadgestatus +wherersvpstatus +attendeeRecords+ ORDERBY_CASE;
					}
				}
				...
				if (!isFilter) {
					...
					if (filter_by.equals("CHECKED IN")) {
						whereClause_name = WHERE_EVENTID +" AND Attendee_Id IN "
								+ innerQueryString("true") + wherebadgestatus +wherersvpstatus +attendeeRecords+ " ORDER BY "+DBFeilds.ATTENDEE_COMPANY+" COLLATE NOCASE";
					} else if (filter_by.equals("ALL"))
						whereClause_name = WHERE_EVENTID + wherebadgestatus +wherersvpstatus +attendeeRecords+ " ORDER BY "+DBFeilds.ATTENDEE_COMPANY+" COLLATE NOCASE ";
					else if (filter_by.equals("CHECKED OUT")) {
						whereClause_name = WHERE_EVENTID +" AND Attendee_Id IN "
								+ innerQueryString("false") + wherebadgestatus +wherersvpstatus +attendeeRecords+ " ORDER BY "+DBFeilds.ATTENDEE_COMPANY+" COLLATE NOCASE ";
					} else if (filter_by.equalsIgnoreCase("NOT REPORTED")) {
						whereClause_name = WHERE_EVENTID +" AND Attendee_Id NOT IN "
								+ innerQueryString("true") + wherebadgestatus +wherersvpstatus +attendeeRecords+ " ORDER BY "+DBFeilds.ATTENDEE_COMPANY+" COLLATE NOCASE ";
					}
				...
				if (!isFilter) {
					...
					if (filter_by.equals("CHECKED IN")) {
						...
						whereClause_name = WHERE_EVENTID +" AND isCheckin= '" + "true"
								+ "'" + wherebadgestatus +wherersvpstatus +attendeeRecords+ ORDERBY_CASE;
					} else if (filter_by.equals("ALL")) {
						whereClause_name = WHERE_EVENTID + wherebadgestatus +wherersvpstatus +attendeeRecords+ ORDERBY_CASE;
					} else if (filter_by.equals("NOT CHECKED IN")) {
	...
	public void setCustomContentView(int layout) {
		...
		if (Util.db.getGroupCount(checked_in_eventId) > 0) {
			...
			img_camera.setImageResource(R.drawable.scan);
		}
		...
		top_layout.setBackgroundResource(R.color.green_top_header);
		back_layout.setBackgroundResource(R.color.green_top_header);
		img_addticket.setImageResource(R.drawable.plus);
		...
		buyers_list = (ListView) linearview.findViewById(R.id.buyerlistview);
		attendee_view = (ListView) linearview.findViewById(R.id.attendee_listView);
		...
		//if (!isEventAdmin()) {
		//TODO for pagersample class
		/*img_setting.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent i = new Intent(AttendeeListActivity.this, AttendeePagerSampleActivity.class);
				startActivity(i);
		...
		img_setting.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
               /* Intent i=new Intent(AttendeeListActivity.this,Registrationclass.class);
                startActivity(i);*/
				if(session_id !=null && session_id.length() >0){
					requestType = "CheckInCountRefresh";
					if (isOnline()) {
		...
		setdisplaytabs();
		/*List<RegistrationSettingsController> reg_settingsbuyer = Util.db
				.getRegSettingsList("where " + DBFeilds.REG_EVENT_ID + "='" + checked_in_eventId + "' AND "
						+ DBFeilds.REG_SETTING_TYPE + "='ScanAttendeeapp'" + " AND " +DBFeilds.REG_LABEL_NAME+"='Display Buyers'");
		List<RegistrationSettingsController> reg_settingatendee = Util.db
				.getRegSettingsList("where " + DBFeilds.REG_EVENT_ID + "='" + checked_in_eventId + "' AND "
						+ DBFeilds.REG_SETTING_TYPE + "='ScanAttendeeapp'" + " AND " +DBFeilds.REG_LABEL_NAME+"='Display Attendees'");
		if(!reg_settingsbuyer.get(0).Included__c &&!reg_settingatendee.get(0).Included__c){
			tablayout.setVisibility(View.GONE);
			attendee_tab.setVisibility(View.GONE);
			buyer_tab.setVisibility(View.GONE);
			buyers_list.setVisibility(View.GONE);
			attendee_view.setVisibility(View.GONE);
		}else if (!reg_settingsbuyer.get(0).Included__c) {
				buyer_tab.setVisibility(View.GONE);
				OntabClick(false);
		} else if (!reg_settingatendee.get(0).Included__c) {
				attendee_tab.setVisibility(View.GONE);
				OntabClick(true);

		}*/
		/*for (RegistrationSettingsController settings : reg_settings) {
			if(settings.Label_Name__c.equalsIgnoreCase("Display Buyers")&&settings.Label_Name__c.equalsIgnoreCase("Display Attendees")){
				attendee_tab.setVisibility(View.GONE);
				buyer_tab.setVisibility(View.GONE);
				buyers_list.setVisibility(View.GONE);
				attendee_view.setVisibility(View.GONE);

			}else if (settings.Label_Name__c.equalsIgnoreCase("Display Buyers")) {
	...
	public void onTstatusrecieved(WebsocketResponseObj websocketResponseObj) {
		runOnUiThread(new Runnable() {
			@Override
			public void run() {
				...
				try {
					...
					if(websocketResponseObj.payload.Event_Id__c.equalsIgnoreCase(checked_in_eventId)) {
						...
						if (isFreeSession) {
							...
							sessionCheckIns = websocketResponseObj.payload.totalItemschkQty__c;
							Util.db.insertandupdatecheckincounts(websocketResponseObj.payload.BLN_Group__c,checked_in_eventId,sessionCheckIns);
	...
	@Override
	public void onPrintstatusrecieved(WebsocketResponseObj websocketResponseObj) {
		if(websocketResponseObj.payload.Event_Id__c.equalsIgnoreCase(checked_in_eventId)) {
	...
	private class ViewHolder {
		TextView attName, attComp, att_checkedin, ticketType, txtseatno,txtrsvpstatus,txtrsvpheading, txt_att_checkin, txt_delete, txt_image,att_badgestatus,txt_badgelabel;
		Button count;
		CheckBox checkBox;
	...
	private class AttendeeAdapter extends CursorAdapter {
		...
		public View newView(Context context, Cursor cursor, ViewGroup parent) {
			try {
				...
				holder.checkBox = (CheckBox) v.findViewById(R.id.check_print);
				holder.statusbar = (FrameLayout) v.findViewById(R.id.statusbar);
				holder.txtseatno = (TextView) v.findViewById(R.id.txtseatnum);
				holder.txtrsvpstatus = (TextView) v.findViewById(R.id.txtrsvpstatus);
				holder.txtrsvpheading = (TextView) v.findViewById(R.id.txt_rsvpheading);
				...
				holder.txtrsvpstatus.setTypeface(Util.roboto_regular, Typeface.BOLD);
...
*/
			if(!NullChecker(c.getString(c.getColumnIndex(DBFeilds.ATTENDEE_IMAGE))).isEmpty()){
				...
				/*Picasso.with(AttendeeListActivity.this).load(url+"&id="+c.getString(c.getColumnIndex(DBFeilds.ATTENDEE_IMAGE)))
			...
			}else if (!Util.NullChecker(c.getString(c.getColumnIndex("CheckedInDate"))).isEmpty()){
				String checkin_time = Util.change_US_ONLY_DateFormatWithSEC(NullChecker(c.getString(c.getColumnIndex("CheckedInDate")) ), checkedin_event_record.Events.Time_Zone__c);
				holder.att_checkedin.setText(checkin_time);
			}
			...
			if (!NullChecker(c.getString(c.getColumnIndex(DBFeilds.ATTENDEE_RSVP_STATUS))).isEmpty()) {
				holder.txtrsvpheading.setVisibility(View.VISIBLE);
				holder.txtrsvpstatus.setVisibility(View.VISIBLE);
				if(c.getString(c.getColumnIndex(DBFeilds.ATTENDEE_RSVP_STATUS)).equalsIgnoreCase("Attending")){
					holder.txtrsvpstatus.setTextColor(getResources().getColor(R.color.green_color));
				}else {
					holder.txtrsvpstatus.setTextColor(getResources().getColor(R.color.orange_bg));
				}
				holder.txtrsvpstatus.setText( c.getString(c.getColumnIndex(DBFeilds.ATTENDEE_RSVP_STATUS)));
			} else {
				holder.txtrsvpheading.setVisibility(View.GONE);
				holder.txtrsvpstatus.setVisibility(View.GONE);
			}
			...
			if (!NullChecker(session_id).trim().isEmpty()&&!isFreeSession) {
				...
				public void onClick(View v) {
					try {
						...
						if(attendeeRecords.equalsIgnoreCase(" GROUP BY "+DBFeilds.ATTENDEE_EMAIL_ID+","+DBFeilds.ATTENDEE_FIRST_NAME+","+DBFeilds.ATTENDEE_LAST_NAME+" ")&&totalrecords>1) {
							Intent i = new Intent(AttendeeListActivity.this, UniqueAttendeesListActivity.class);
						...
						//if (tickets_count > 1) {
							...
							Intent i = new Intent(AttendeeListActivity.this, MergeLevelAttendeesListActivity.class);
							...
							showScannedTicketsAlert("Please TurnON at least one session for scanning.", true);
						} else if(!NullChecker(session_id).equalsIgnoreCase(Util.db.getSwitchedONGroupId(checked_in_eventId))&&!NullChecker(session_id).isEmpty()){
							showScannedTicketsAlert("Please TurnON "+session_name+" for scanning.", true);
							...
							if(ischeckin && Util.checkin_only_pref.getBoolean(sfdcddetails.user_id+checked_in_eventId, false)){
								...
								openScanSettingsAlert(AttendeeListActivity.this,attendee_itempool_id,
			...
			holder.checkBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
				...
				public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
					try {
						...
						c_new = c;
						if (isChecked) {
							String tstatus = Util.db.getTStatusBasedOnGroup(c_new.getString(c_new.getColumnIndex("Attendee_Id")), c_new.getString(c_new.getColumnIndex(DBFeilds.ATTENDEE_ITEM_POOL_ID)), checked_in_eventId, Util.db.getSwitchedONGroupId(checked_in_eventId));
							String orderstatus = Util.db.getOrderStatuswithAttendee(c_new.getString(c_new.getColumnIndex(DBFeilds.ATTENDEE_ORDER_ID)), checked_in_eventId);
							String item_pool_id = Util.db.getItemPoolID(c_new.getString(c_new.getColumnIndex("Attendee_Id")));
							if(Util.db.getSwitchedOnScanItem(checked_in_eventId).isEmpty()){
								...
								holder.checkBox.setChecked(false);
								openScanSettingsAlert(AttendeeListActivity.this,item_pool_id,
										AttendeeListActivity.class.getName());
								Util.txt_dismiss.setVisibility(View.GONE);
								//showCustomToast(AttendeeListActivity.this, "Sorry! You are not allowed to check-in for \"" + item_pool_name + "\".", R.drawable.img_like, R.drawable.toast_redrounded, false);
								...
								Util.txt_okey.setOnClickListener(new OnClickListener() {
									...
									public void onClick(View v) {
										Util.alert_dialog.dismiss();
										startCheckclickupdate(holder.checkBox, true, c_new);
									}
								});
								Util.txt_dismiss.setOnClickListener(new OnClickListener() {
									@Override
			...
			holder.image_attendee_delete.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (!isOnline()) {
						...
						final AlertDialogCustom dialog = new AlertDialogCustom(AttendeeListActivity.this);
	...
	public void customAskDialog(String title, String message, final String eventId, final String attendeeId,
		...
		AlertDialog.Builder alertDialog = new AlertDialog.Builder(AttendeeListActivity.this, R.style.CustomForDialog);
	...
	public void openFilterDialog(Context ctx) {
		try {
			...
			final Spinner spnr_rsvp_status =(Spinner) sort_dialog.findViewById(R.id.spnr_rsvp_status);
			...
			ArrayAdapter<String> adapter = new ArrayAdapter<String>(ctx, android.R.layout.simple_dropdown_item_1line,item_pool_names);
			...
			rsvpstatus_adapter=new ArrayAdapter<String>(this, android.R.layout.simple_spinner_dropdown_item, Util.db.getRSVPStatusFiterItems(checked_in_eventId,DBFeilds.ATTENDEE_RSVP_STATUS));
			spnr_rsvp_status.setAdapter(rsvpstatus_adapter);
			int k=0;
			for(String key : Util.db.getRSVPStatusFiterItems(checked_in_eventId,DBFeilds.ATTENDEE_RSVP_STATUS)){
				if(rsvpfilter_by.equalsIgnoreCase(key)){
					spnr_rsvp_status.setSelection(k,false);
					break;
				}
				k++;
			}
			...
			btn_done.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					String rsvp_status = (String) spnr_rsvp_status.getSelectedItem().toString();
...
*/
			...
			SessionSpnrAdapter session_adapter = new SessionSpnrAdapter(AttendeeListActivity.this, group_list);
			...
			spnr_ticket_names.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if(spnr_session_name.getSelectedItemPosition()==0){
						showCustomToast(AttendeeListActivity.this, "Please Select any Session for Session Tickets!", R.drawable.img_like,R.drawable.toast_redrounded,false);
					}else {
			...
			spnr_ticket_status.setOnItemSelectedListener(new OnItemSelectedListener() {
				...
				public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
					if(spnr_session_name.getSelectedItemPosition()==0&&spnr_ticket_status.getSelectedItemPosition()>0){
						spnr_ticket_status.setSelection(0);
						showCustomToast(AttendeeListActivity.this, "Please Select any Session for Status!", R.drawable.img_like,R.drawable.toast_redrounded,false);
					}
				}
			...
			btn_done.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					badgefilter_by=badge_status;
					rsvpfilter_by=rsvp_status;
					if (NullChecker(session_id).isEmpty() && NullChecker(ticket_status).equalsIgnoreCase("ALL")	&& NullChecker(item_pool_id).isEmpty()
							&&NullChecker(badge_status).equalsIgnoreCase("ALL")&&NullChecker(rsvp_status).equalsIgnoreCase("ALL")) {
						UpdateAttendeeCursor();
					} else {
						isFilter = true;
						SortFunction(R.id.txtsort1, is_buyer);
	...
	public void onClick(View v) {
		try {
			...
			if (v == back_layout) {
				...
				/*if(!NullChecker(session_id).isEmpty()){
					openFilterDialog(AttendeeListActivity.this);
				}

				else*/ if (!Util.db.getTicketedGroupList(checked_in_eventId).isEmpty()) {
					openFilterDialogNew(AttendeeListActivity.this, true,false);
				} else {
					/*AlertDialogCustom dialog = new AlertDialogCustom(AttendeeListActivity.this);
					dialog.setParamenters("Alert", "Please select at least one session.", null, null, 1, false);
					dialog.show();*/
					openFilterDialog(AttendeeListActivity.this);
					//openFilterDialogNew(AttendeeListActivity.this, true,true);
				}
			} else if (v == sort_layout) {
				openSortDialog(AttendeeListActivity.this);
			}
		} catch (Exception e) {
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK) {
			if (!NullChecker(activity_name).equalsIgnoreCase(DashboardActivity.class.getName())) {
				Intent startentent = new Intent(AttendeeListActivity.this, DashboardActivity.class);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == Util.DASHBORD_ONACTIVITY_REQ_CODE) {
			Intent i = new Intent(AttendeeListActivity.this, SplashActivity.class);
			startActivity(i);
			finish();
	...
	private void setdisplaytabs(){
		try {
			...
			if (!reg_settingsbuyer.get(0).Included__c && !reg_settingatendee.get(0).Included__c) {
				...
				buyers_list.setVisibility(View.GONE);
				attendee_view.setVisibility(View.GONE);
				txt_title.setText("No Tab to Display");
				txt_noattendee.setVisibility(View.VISIBLE);
			} else if (!reg_settingsbuyer.get(0).Included__c) {
				buyer_tab.setVisibility(View.GONE);
				OntabClick(false);
	...
	public void setAttendeeCheckinAndCheckout() {
		...
		if (lay_SessioHeadder.getVisibility() == View.VISIBLE) {
			...
			if(isOnline()){
				...
				String toatlCheckint = sessionCheckIns;

				/*if(toatlCheckint.equals("0/0")){
					toatlCheckint = "-";
				}*/
				txt_total_checkin.setText("(Total Check-ins: "+totalCheckIn+"/" + totalCount+")");
		...
		public View getView(int position, View converterView, ViewGroup viewGroup) {
			try {
				...
				if (converterView == null) {
					...
					viewHolder.status_bar =(View) converterView.findViewById(R.id.status_bar);
					//viewHolder.btn_status =(Button)converterView.findViewById(R.id.btn_status);
			...
			/*if (!NullChecker(offlineobj.error).isEmpty()) {
				...
				if (!NullChecker(Util.db.getAttendeeCompany(offlineobj.badge_id)).isEmpty()) {
					if(Util.db.isAttendeeValidForSession(offlineobj.badge_id)) {
						viewHolder.status_bar.setBackgroundColor(getResources().getColor(R.color.green_connected));
						/*viewHolder.btn_status.setBackgroundResource(R.drawable.green_price_bg);
						viewHolder.btn_status.setText("Check-In");*/
					}
	...
	/*private void connectWebSocket() {
		URI uri;
		try {
			//uri = new URI("ws://peprod.eventdex.com/ws");//ws://peweb.eventdex.com/ws
			uri = new URI("ws://peweb.eventdex.com/ws");//

			//uri = new URI("wss://chat.eventdex.com/socket.io/");
		...
		mWebSocketClient = new WebSocketClient(uri) {
			...
			public void onOpen(ServerHandshake serverHandshake) {
				...
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						showCustomToast(AttendeeListActivity.this,"WEBsocket connection opened",R.drawable.img_like,R.drawable.toast_greenroundededge,false);
					}
				});
				//mWebSocketClient.send("Hello from sai");
			}


			@Override
			public void onMessage(String response) {
				Log.i("Websocket", "onmessage " + response);
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						gson = new Gson();
						WebsocketResponseObj websocketResponseObj =(WebsocketResponseObj)gson.fromJson(response, WebsocketResponseObj.class);
						if(websocketResponseObj.channel.equalsIgnoreCase("/event/PL_BLN_Tstatus__e")){
							boolean isFreeSession = false;
							isFreeSession = Util.db.isItemPoolFreeSession(websocketResponseObj.payload.BLN_Item_Pool_Id__c, checked_in_eventId);
							...
							//updateView();
							updateCheckinView();
						}else  if(websocketResponseObj.channel.equalsIgnoreCase("/event/PL_PrintStatus__e")){
							String printstatus = websocketResponseObj.payload.Status__c;
							String TicketId = websocketResponseObj.payload.Ticket_Id__c;
							String lastmodifieddate = websocketResponseObj.payload.CreatedDate;
							Util.db.insertandupdateAttendeeBadgeIdandPrintstatus(TicketId,printstatus,lastmodifieddate);
						...
						showCustomToast(AttendeeListActivity.this, "Received data ", R.drawable.img_like,R.drawable.toast_greenroundededge,false);
			...
			public void onClose(int i, String s, boolean b) {
				...
				runOnUiThread(new Runnable() {
					@Override
					public void run() {showCustomToast(AttendeeListActivity.this,"closed",R.drawable.img_like,R.drawable.toast_redrounded,false);
					}
				});
			}
			...
			public void onError(Exception e) {
				...
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						showCustomToast(AttendeeListActivity.this,"Error",R.drawable.img_like,R.drawable.toast_redrounded,false);
					}
				});
			}
		};
	...
	private void dissconnectIOSocket(){
		try {
			...
			mSocket.off("/event/PL_BLN_Tstatus__e",onNewCheckins);
			mSocket.off("/event/PL_PrintStatus__e",onPrintStatus);
			//mSocket.isActive();
		} catch (URISyntaxException e) {
			e.getMessage();
		}
				...
				public void run() {
					...
					try {
						...
						if(NullChecker(websocketResponseObj.payload.Event_Id__c).equalsIgnoreCase(checked_in_eventId)) {
							...
							//dashboardActivity.onTstatusrecieved(websocketResponseObj);
							if(websocketResponseObj.payload.BLN_Group__c.equalsIgnoreCase(NullChecker(session_id))) {
								sessionCheckIns = websocketResponseObj.payload.totalItemschkQty__c;
								filter_by="";
								//SortFunction(R.id.txtsort1, false);
								updateCheckinViewforSocket();
								//updateView();
							}else if(websocketResponseObj.payload.BLN_Group__c.equalsIgnoreCase(Util.db.getSwitchedONGroupId(checked_in_eventId))){
								sessionCheckIns = websocketResponseObj.payload.totalItemschkQty__c;
								filter_by="";
								updateCheckinViewforSocket();
								//SortFunction(R.id.txtsort1, false);
	...
	public void startCheckclickupdate(CheckBox checkBox, Boolean isChecked, Cursor attendee_cursor){
		boolean isFreeSession = false;
		List<ScannedItems> scanticks = Util.db.getSwitchedOnScanItem(checked_in_eventId);
		if(scanticks.size() > 0){
			...
			if(Boolean.valueOf(status) && Util.checkin_only_pref.getBoolean(sfdcddetails.user_id+checked_in_eventId, false)){
				...
				String check_box_value = String.valueOf(checkBox.isChecked());
			...
			if (NullChecker(status).equalsIgnoreCase("true")) {
				...
				//checkBox.setBackgroundResource(R.drawable.checkmark_h);
			} else {
				checkBox.setChecked(false);
				//checkBox.setBackgroundResource(R.drawable.checkmark_n);
			}
			//openScanSettingsAlert(AttendeeListActivity.this,attendee_cursor.getString(attendee_cursor.getColumnIndex(DBFeilds.ATTENDEE_ITEM_POOL_ID)),AttendeeListActivity.class.getName());
		}else{
			String ticket_id = attendee_cursor.getString(attendee_cursor.getColumnIndex("Attendee_Id"));
			String status = Util.db.getTStatusBasedOnGroup(attendee_cursor.getString(attendee_cursor.getColumnIndex(DBFeilds.ATTENDEE_ID)), attendee_cursor.getString(attendee_cursor.getColumnIndex(DBFeilds.ATTENDEE_ITEM_POOL_ID)), checked_in_eventId);
